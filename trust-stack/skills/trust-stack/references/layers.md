...
|---|---|---|
| `shape` | The wrong change is hard to represent. When `bend` is on PATH, the critical claims are `trust-stack/bend/LAWS.bend` and the gate is `bend PROOF.bend`. | The bug is legal code, and a person is asked to notice. |
