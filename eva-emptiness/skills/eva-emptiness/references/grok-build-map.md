...
| Tether | PreToolUse → `bin/eva-tether-pretool.sh` (C `bin/eva-tether` preferred; shell fallback); `--max-turns` / Rhai `max_turns`; `/rewind` | Bound surprise; deny/ask trauma bash — [permission-tether.md](permission-tether.md) |
| Tether compile | `/eva-tether-init` · `/eva-tether-init --yes` | Consent-only C11 build; no network/sudo |
...
**Refuse under EVA:** `--always-approve`, `--yolo`, `permission_mode=always-approve`; force-push / `reset --hard` / `rm -rf /`. Ordinary `git push`: Grok deny (HITL), Cursor ask. Plan mode does **not** gate bash writes — C/shell tether covers destructive shell.
